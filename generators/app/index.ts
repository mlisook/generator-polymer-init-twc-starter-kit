...
class GeneratorStarterKitTsTwc extends Generator {
    ...
    writing() {
        ...
        this.fs.copyTpl(
            this.templatePath('src/_element.ts'),
            `src/${elementName}.ts`,
            this.props);
        if (this.props.templateLocation == 'inHTML') {
            this.fs.copyTpl(
                this.templatePath('src/_element.template.html'),
                `src/${elementName}.template.html`, this.props);
        }

        // copy the icons element
        this.fs.copyTpl(
            this.templatePath('src/_icons.html'),
            `src/${this.props.viewPrefix}-icons.html`,
            this.props);

        // copy the views
        const vSfx: Array<string> = ['1', '2', '3', '404'];
        vSfx.forEach((sfx) => {
            const viewName: string = this.props.viewPrefix + "-view" + sfx;
            this.props["elementClassName" + sfx] = viewName.replace(
                /(^|-)(\w)/g,
                (_match: string, _p0: string, p1: string) => p1.toUpperCase());
            this.fs.copyTpl(
                this.templatePath('src/_view' + sfx + '.ts'),
                `src/${viewName}.ts`,
                this.props);
            if (this.props.templateLocation == 'inHTML') {
                this.fs.copyTpl(
                    this.templatePath(`src/_view${sfx}.template.html`),
                    `src/${viewName}.template.html`, this.props);
            }
        });
        ...
        this.fs.copyTpl(
            ...
            this.props);
        this.fs.copyTpl(
            this.templatePath('test/_view1.html'),
            `${this.destinationPath()}/test/${this.props.viewPrefix}-view1.html`,
            this.props);
        this.fs.copy(
